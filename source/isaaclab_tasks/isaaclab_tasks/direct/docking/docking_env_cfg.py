"""Configuration definitions for the docking environment."""
...
from .docking_tasks_cfg import ASSET_DIR, DockTask
...
@configclass
class DockingObsRandCfg:
    fixed_asset_pos = [0.001, 0.001, 0.001]


@configclass
class DockingCtrlCfg:
    ema_factor = 0.2
...
@configclass
class DockingEnvBaseCfg(DirectRLEnvCfg):
    decimation = 8
    action_space = 6
    ...
    task_name: str = "dock"
    task: DockTask = _DOCK_TASK
    obs_rand: DockingObsRandCfg = DockingObsRandCfg()
    ctrl: DockingCtrlCfg = DockingCtrlCfg()
...
@configclass
class DockingEnvCfg(DockingEnvBaseCfg):
    task_name: str = "dock"
    task: DockTask = _DOCK_TASK
    episode_length_s: float = _DOCK_TASK.duration_s
